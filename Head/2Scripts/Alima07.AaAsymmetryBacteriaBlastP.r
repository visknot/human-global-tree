...
data = read.table("../../Body/1Raw/AminoAcidsHumanVsBacteria.csv", sep = ';', header = TRUE, quote = '')
data = data[data$Organism != 'Human',]
data = data[-c(1,24)]
data = data[data$Gene != 'ND6',]
...
Agg1 = data.frame(sum(data$Gainers),sum(data$Loosers),sum(data$Intermediate),sum(data$All))
names(Agg1) = c('Gainers','Loosers','Intermediate','All')
