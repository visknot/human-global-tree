...
FromTo = aggregate(data$FreqOfAASub, by = list(data$FromTo), FUN = sum)
...
Short = Short[,-c(2:6)]
write.table(Short,"../../Body/3Results/Alima04.AsymmetryInCancers.txt")
dev.off()

### compare humans and vertebrates: !!!!!!!!!!! CAN'T MERGE BECAUSE OF LEU AND SER!!!!!
...
Short = merge(Short,ShortHumans)
cor.test(Short$ExpectedMoreThanOne,Short$Humans.ExpectedMoreThanOne, method = 'spearman') # non significant. May be will be significant with mammals
