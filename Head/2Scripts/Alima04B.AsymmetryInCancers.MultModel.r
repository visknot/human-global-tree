...
MitAnno = MitAnno[MitAnno$AaSub!='',] # 7155
MitAnno = merge(MitAnno,AaPredictions,by="AaSub") # 7146 (catch the difference!!!!!!!!!!! Victor's AGA  in ND6 is translated as stop, but it should be  TCT (Ser)
...
nrow(agg) # 7160
length(unique(agg$position)) # 7159 !!!!!!!!!!!!!!!!!!! should be the same as line above!!!!!!!!
...
summary(glm(mutation ~ ExpDummyFromLoserToGainer + TimeBeingSingleStranded + GranthamDistance + ChToThDummy + DummyNd6, data = agg, family = 'poisson'))
summary(glm(mutation ~ (ExpDummyFromLoserToGainer + TimeBeingSingleStranded + GranthamDistance + ChToThDummy)*DummyNd6, data = agg, family = 'poisson'))
